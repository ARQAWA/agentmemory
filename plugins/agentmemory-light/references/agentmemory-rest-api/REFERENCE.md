...
HTTP requests use the same configured adapter command, args, proxy, and auth as
MCP. Use the `requestHttp` launcher described in `SKILL.md`; do not assume a
local address or create a direct proxy client.
